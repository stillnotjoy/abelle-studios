...
const socialLinks = [
  ...
  { name: "Instagram", label: "Instagram", href: "https://www.instagram.com/abellestudios", icon: "instagram" },
  { name: "TikTok", label: "TikTok", href: "https://www.tiktok.com/@abellestudios", icon: "tiktok" },
];
...
const packages = [
  {
    ...
    guide: "/assets/personal-guide.svg",
    price: "₱499",
    inclusions: [
    ...
    note: "Discount rates for students are available.",
  },
  {
    id: "02",
    title: "Duo Portraits",
  ...
  },
  {
    ...
    guide: "/assets/barkada-guide.svg",
    price: "₱999",
    inclusions: [
      "Good for 3–4 people",
    ...
    <main className="site">
      ...
      <header className="nav">
        <a className="brand" href="#top" aria-label="Abelle Studios home">
          <img src="/assets/logo.svg" alt="Abelle Studios" className="brand-logo" />
      ...
      <section id="top" className="hero section-wrap">
        ...
        <div className="hero-visual" aria-label="Abelle Studios photo preview">
          ...
          <button className="floating-guide" onClick={() => setActiveImage("/assets/overview.svg")}>
            <small>Current Guide</small>
            <strong>Tap to enlarge</strong>
          </button>
        </div>
      </section>
        ...
        <div className="package-grid">
          {packages.map((pkg) => (
            <article className="package-card" key={pkg.id}>
              <button className="package-media" onClick={() => setActiveImage(pkg.guide)}>
          ...
          <article className="custom-card">
            <img src="/assets/newborn.jpg" alt="Newborn photography sample" />
            <div>
        ...
        <div className="section-wrap contact-grid">
          <div>
            ...
            <div className="contact-info">
              ...
              <div><small>Social</small><span>Abelle Studios</span></div>
            </div>
          ...
          <aside className="quote-card">
            <img src="/assets/logo.svg" alt="Abelle Studios logo" />
      ...
      <footer className="footer">
        <div className="footer-brand">
          <img src="/assets/logo.svg" alt="Abelle Studios" />
          <div>
      ...
      {activeImage && (
        <div className="modal" onClick={() => setActiveImage(null)}>
          <button className="modal-close" onClick={() => setActiveImage(null)}>Close</button>
          <img src={activeImage} alt="Expanded Abelle Studios guide" />
        </div>
      )}
    </main>
  );
...
.brand-logo { width: 58px; height: 42px; object-fit: contain; display: block; }
...
h1, h2, h3 { font-family: 'Cormorant Garamond', Georgia, serif; font-weight: 300; margin: 0; letter-spacing: -.025em; }
...
.floating-guide { position: absolute; right: 18px; bottom: 28px; background: rgba(255,255,255,.88); backdrop-filter: blur(12px); padding: 14px 18px; border: 1px solid var(--faint); box-shadow: 0 14px 32px rgba(0,0,0,.08); cursor: zoom-in; text-align: left; }
.floating-guide small { display: block; color: var(--muted); font-size: 11px; text-transform: uppercase; letter-spacing: .15em; }
.floating-guide strong { font-family: 'Cormorant Garamond', Georgia, serif; font-size: 20px; font-weight: 400; }
...
.package-card { background: var(--paper); border: 1px solid var(--faint); box-shadow: 0 12px 34px rgba(0,0,0,.045); display: grid; grid-template-columns: 42% 58%; min-height: 560px; overflow: hidden; }
.package-media { border: 0; padding: 0; background: var(--soft); cursor: zoom-in; position: relative; overflow: hidden; }
.package-media img { width: 100%; height: 100%; object-fit: cover; display: block; transition: transform .35s ease; }
.package-media:hover img { transform: scale(1.035); }
.package-media span { position: absolute; left: 14px; bottom: 14px; background: rgba(255,255,255,.86); padding: 8px 10px; color: var(--ink); text-transform: uppercase; letter-spacing: .14em; font-size: 10px; }
.package-content { padding: 30px; display: flex; flex-direction: column; }
...
.package-footer { margin-top: auto; padding-top: 20px; display: flex; justify-content: space-between; align-items: flex-end; gap: 12px; }
.package-footer small { color: #999; text-transform: uppercase; letter-spacing: .12em; font-size: 10px; display: block; }
.package-footer strong { font-family: 'Cormorant Garamond', Georgia, serif; font-size: 42px; line-height: 1; }
.package-footer em { color: var(--gold); font-size: 11px; max-width: 130px; text-align: right; }
.package-content .btn { margin-top: 22px; }

.custom-card { grid-column: 1 / -1; background: var(--dark); color: var(--bg); min-height: 360px; display: grid; grid-template-columns: 36% 1fr; overflow: hidden; }
.custom-card img { width: 100%; height: 100%; object-fit: cover; filter: saturate(.9); }
.custom-card > div { padding: 42px; display: flex; flex-direction: column; justify-content: center; }
...
.quote-card img { width: 170px; max-width: 80%; margin: 0 auto 18px; display: block; }
...
.footer-brand img { width: 68px; height: 52px; object-fit: contain; filter: invert(1); opacity: .9; }
...
.modal { position: fixed; inset: 0; background: rgba(0,0,0,.86); z-index: 100; padding: 70px 16px 24px; display: grid; place-items: start center; overflow: auto; cursor: zoom-out; }
.modal img { width: min(880px, 100%); background: white; box-shadow: 0 30px 90px rgba(0,0,0,.45); }
.modal-close { position: fixed; top: 18px; right: 18px; border: 1px solid rgba(255,255,255,.3); background: rgba(255,255,255,.12); color: white; padding: 10px 16px; border-radius: 999px; cursor: pointer; }

@media (max-width: 980px) {
  .hero, .contact-grid { grid-template-columns: 1fr; }
  .hero-visual { max-width: 520px; }
  .package-grid { grid-template-columns: 1fr; }
  .package-card { grid-template-columns: 260px 1fr; min-height: auto; }
...
@media (max-width: 720px) {
  ...
  .floating-guide { position: static; width: 100%; margin-top: 10px; }
  .services-intro, .packages-section, .why-section, .promo-section, .contact-section { padding: 46px 0; }
  .package-card, .custom-card { grid-template-columns: 1fr; }
  .package-media img { max-height: 420px; }
  .package-content, .custom-card > div { padding: 26px 22px; }
  .package-footer { display: block; }
  .package-footer em { display: block; max-width: none; text-align: left; margin-top: 8px; }
